...
**核心的な差別化**: 既存の AI ライティングツール（Transcope, AIブログくん等）は「1 記事ずつ作成」モデルだが、blog-factory SaaS は「複数ブログ × 複数テーマを定期スケジュールで一括自動化」するオペレーション全体を自動化する点が異なる。

## 解決する課題

### 課題 1: 技術的参入障壁

blog-factory の現システムは GitHub Actions の設定・API キーの管理・シェルスクリプトの理解が必要で、エンジニア以外は利用できない。日本のブロガー（推計 600 万人のアクティブユーザー）の大多数は非エンジニアであり、このシステムの恩恵を受けられていない。
...
| セグメント | 特徴 | ペインポイント | 規模 |
|-----------|------|-------------|------|
| 副業アフィリエイター | 会社員・主婦。月 1〜10 万円の副収入目標 | 記事更新に時間がない。技術知識なし | 日本で約 93 万人（本格的アフィリエイター推計） |
| 個人ブロガー | ゲーム・趣味テーマ。ファン収益化目標 | 継続更新が困難。SEO 対策の知識不足 | 日本のアクティブブロガー推計 600 万人の一部 |
...
|------|-------------|-----------|----------------|-------------------|
| 記事生成 | キーワード → 1 記事 | キーワード → 1 記事（SEO強化） | 手動プロンプト | キーワードリスト → 複数記事一括 |
| WordPress 自動投稿 | あり | なし（手動コピペ） | なし | あり（複数ブログ対応） |
| スケジュール自動実行 | なし（手動トリガー） | なし | なし | あり（日次・週次） |
| 複数ブログ一元管理 | なし（1 ブログ） | なし | なし | あり（最大 10 ブログ） |
| コスト（月 50 記事） | 約 16,500 円 | 約 11,000 円〜 | 約 1,000 円（時間コスト別） | 約 4,980 円（想定） |
| 技術知識要否 | 不要 | 不要 | 不要 | 不要 |
| テーマ別収益最適化 | なし | なし | なし | あり（テーマ × 収益性分析） |

**ブルーオーシャン機会**: 「複数ブログ × スケジュール自動化 × 収益最適化レポート」を一体提供するツールは、日本市場に存在しない。
...
**コスト構造（Growth プランの場合）:**
- Claude Haiku 4.5 API: 1 記事あたり約 15 円 × 60 記事 = 約 900 円
- サーバー（Vercel + Supabase 無料枠〜有料）: 約 1,000 円
- GitHub Actions（記事生成実行基盤）: 約 500 円
- **合計コスト**: 約 2,400 円
- **粗利**: 4,980 − 2,400 = 2,580 円（粗利率 51.8%）

### 損益分岐点

- Growth プラン単独での BEP: 初期固定費（開発 + インフラ）が月 10 万円と仮定した場合、**約 39 契約で黒字化**
- Starter プラン換算: 約 86 契約
...
| SAM（日本 AI ライティングツール市場・推計） | 日本のアクティブブロガー 600 万人 × 想定利用率 1% × ARPU 4,980 円 = 月約 2.99 億円 | 推定（ブロガー人口: [bonbankblog.com](https://bonbankblog.com/bloger-population)） | 2026-02-22 |
...
| 日本のアクティブブロガー推計 | 約 600 万人 | [bonbankblog.com](https://bonbankblog.com/bloger-population) | 2026-02-22 |
| 日本の本格的アフィリエイター | 約 93 万人 | [株式会社もしも リリース](https://www.moshimo.co.jp/news/release/detail?id=123) | 2026-02-22 |
...
### コスト試算（月額、Growth プラン想定）

| 項目 | 費用 |
|------|------|
| Vercel Pro（チーム機能含む） | $20（約 3,000 円） |
| Supabase Pro | $25（約 3,750 円） |
| Claude API（Haiku 4.5）100 ユーザー × 60 記事 | 約 90,000 円 |
| GitHub Actions | 約 5,000 円 |
| **合計（100 ユーザー規模）** | **約 10 万円/月** |

→ 100 ユーザー Growth プランで月収 49.8 万円 / コスト約 10 万円 = **粗利率 約 80%**

**月額 $250 制約について**: 開発初期・MVP フェーズ（〜50 ユーザー）まではコストを $250 以内に収めることが可能。ユーザー増加に比例してコスト・収益ともにスケールするため、制約違反ではない。
...
| 競合（AIブログくん）の機能拡張 | 中 | 「複数ブログ一元管理 + スケジューリング」での差別化を維持。早期ユーザー獲得でスイッチングコストを高める |
...
- [株式会社もしも — アフィリエイト市場規模・人口データ](https://www.moshimo.co.jp/news/release/detail?id=123) — 信頼度: ★★★ — 取得日: 2026-02-22
- [bonbankblog.com — 日本のブログ人口推計](https://bonbankblog.com/bloger-population) — 信頼度: ★★ — 取得日: 2026-02-22
- [矢野経済研究所 — アフィリエイト市場調査 2025](https://www.yano.co.jp/press-release/show/press_id/3746) — 信頼度: ★★★ — 取得日: 2026-02-22
...
- WebFetch: AIブログくん公式サイト、Grand View Research マーケットレポート、Boxil 比較記事、SHIFT AI TIMES
